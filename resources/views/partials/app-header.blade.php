<flux:tab.group class="fixed top-0 right-0 left-0 z-10 min-h-10">
   ...
   <form method="POST" action="{{ route('logout') }}" class="w-full flex items-center">
     ...
     <flux:tabs>
        ...
        <button name="logout" type="submit" class="flex items-center gap-2 text-sm font-bold text-gray-500 opacity-75 hover:text-gray-600">
          <flux:icon name="arrow-right-start-on-rectangle" class="w-4 h-4 font-bold flex justify-center text-gray-500 opacity-75 hover:text-gray-600" />
          Logout
         </button>
     </flux.tabs>
    </form>
